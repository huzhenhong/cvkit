...
namespace cvkit::infer::detail
{
    ...
    namespace
    {
        ...
        __device__ float sample_plane_linear(
            const unsigned char* src,
            std::size_t          src_stride,
            int                  src_width,
            int                  src_height,
        ...
        __device__ float sample_nv12_chroma_linear(
        ...
        __device__ void sample_nv12_rgb_linear(
            const unsigned char* src,
            std::size_t          src_stride,
            int                  src_width,
            int                  src_height,
            ...
            float&               b)
        {
            ...
            r = fminf(fmaxf(raw_r, 0.0F), 255.0F) / 255.0F;
            g = fminf(fmaxf(raw_g, 0.0F), 255.0F) / 255.0F;
            b = fminf(fmaxf(raw_b, 0.0F), 255.0F) / 255.0F;
        }
            ...
            float*               dst)
        {
            ...
            if (x >= pad_x && x < pad_x + resized_width && y >= pad_y && y < pad_y + resized_height)
            {
                ...
                if (nv12_input)
                {
                    sample_nv12_rgb_linear(src, src_stride, src_width, src_height, src_x, src_y, r, g, b);
                }
                else
                {
        ...
        LetterboxResult&                 result)
    {
        ...
        if (!nv12_input && image.frame.desc.channels != 3)
        {
            return false;
        }
