...
bool Yolov8::ReadModel(cv::dnn::Net& net, std::string& netPath, bool isCuda = false)
{
    ...
    {
        SPDLOG_ERROR("model path [{}] do not exist.", netPath);
        return false;
    }
...
bool Yolov8::Detect(cv::Mat&                srcImg,
                    cv::dnn::Net&           net,
                    std::vector<OutputSeg>& output)
{
    ...
    LetterBox(srcImg,
              netInputImg,
              params,
              cv::Size(_netWidth, _netHeight));

    cv::dnn::blobFromImage(netInputImg,
                           blob,
                           1 / 255.0,
                           cv::Size(_netWidth, _netHeight),
                           cv::Scalar(0, 0, 0),
    ...
    cv::Mat               output0 = cv::Mat(cv::Size(net_output_img[0].size[2],
                                       net_output_img[0].size[1]),
                              CV_32F,
                              (float*)net_output_img[0].data)
