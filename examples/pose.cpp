...
int main(int argc, char** argv)
{
    ...
    CLI::App app{"cvkit pose example"};
    app.add_option("--model", model_path, "Path to pose ONNX model")->required();
    ...
    app.add_flag("--async", async_infer, "Run pose through the async submit() path");
    ...
    spec.task         = cvkit::infer::TaskKind::pose;
    spec.family       = "pose";
    ...
    {
        std::cerr << "pose output missing keypoints\n";
        return 1;
    }
    ...
    std::cout
        ...
        << " keypoint_count=" << keypoints->points.size();
    if (scores != nullptr)
    {
